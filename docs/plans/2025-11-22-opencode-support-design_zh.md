...
通过原生 OpenCode 插件架构为 OpenCode.ai 添加完整的超级能力支持，该架构与现有的 Codex 实现共享核心功能。

## 背景

OpenCode.ai 是一个类似于 Claude Code 和 Codex 的编码代理。之前尝试将超级能力移植到 OpenCode（PR #93, PR #116）采用了文件复制方法。本设计采用不同的方法：使用其 JavaScript/TypeScript 插件系统构建原生 OpenCode 插件，同时与 Codex 实现共享代码。

### 平台间关键差异

- **Claude Code**：原生 Anthropic 插件系统 + 基于文件的技能
- **Codex**：无插件系统 → 引导标记 + CLI 脚本
...
---
name: skill-name
description: 当[条件]时使用 - [它做什么]；[额外上下文]
---
```
