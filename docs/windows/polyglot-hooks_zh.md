...
5. `CMDBLOCK` 之后的内容永远不会被 CMD 执行

#### 在 Unix 上（bash/sh）

1. `: << 'CMDBLOCK'` - `:` 是无操作符，`<< 'CMDBLOCK'` 开始一个 heredoc
...
对于具有多个钩子的插件，可以创建一个接受脚本名称作为参数的通用包装器：
