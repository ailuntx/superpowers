...
- 使用递归中点细分算法
- 创建包含测试的 `internal/sierpinski/sierpinski_test.go`：
...
- 将迭代次数映射到字符梯度 " .:-=+*#%@"（如果提供单个字符则使用该字符）
- 创建包含测试的 `internal/mandelbrot/mandelbrot_test.go`：
  - 输出尺寸匹配请求的宽度/高度
...
**执行：**
- 验证谢尔宾斯基的 `--char` 标志将字符传递给算法
- 对于曼德博，`--char` 应使用单个字符而非梯度
- 为自定义字符输出添加测试
