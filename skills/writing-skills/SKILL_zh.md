...
**核心原则：** 如果你没有观察过代理在没有该技能时的失败情况，你就不知道这个技能是否教对了东西。

**必备背景：** 在使用此技能前，你必须理解 superpowers:test-driven-development。该技能定义了基本的 RED-GREEN-REFACTOR 循环。本技能将 TDD 适配到文档编写。

**官方指南：** 关于 Anthropic 官方的技能编写最佳实践，请参阅 anthropic-best-practices.md。本文档提供了额外的模式和指南，以补充本技能中 TDD 重点方法。
...
- 机械性约束（如果可以用正则表达式/验证强制执行，就自动化它——为需要判断的情况保留文档）
...
**为何重要：** 测试发现，当描述总结了技能的工作流时，Claude 可能会遵循描述而不是阅读完整的技能内容。一个描述说“任务间进行代码审查”导致 Claude 只做一次审查，即使技能的流程图清楚地显示了两次审查（规范符合性然后代码质量）。

当描述改为仅“在当前会话中执行具有独立任务的实施计划时使用”（无工作流总结）时，Claude 正确地阅读了流程图并遵循了两阶段审查过程。

**陷阱：** 总结工作流的描述创建了 Claude 会走的捷径。技能主体变成了 Claude 会跳过的文档。

```yaml
# ❌ 差：总结工作流 - Claude 可能遵循此描述而非阅读技能
...
**问题：** 入门和频繁引用的技能会加载到每次对话中。每个令牌都很重要。
...
**为何不使用 @ 链接：** `@` 语法会立即强制加载文件，在你需要它们之前就消耗了 200k+ 的上下文。
...
**必备背景：** superpowers:test-driven-development 技能解释了为何这很重要。相同的原则适用于文档。
...
| 借口 | 现实 |
|--------|---------|
| "技能显然清晰" | 对你清晰 ≠ 对其他代理清晰。测试它。 |
| "这只是参考" | 参考可能有空白、不清晰的部分。测试检索。 |
| "测试过度" | 未经测试的技能总有问题。15 分钟测试节省数小时。 |
| "如果出现问题我会测试" | 问题 = 代理无法使用技能。在部署前测试。 |
| "测试太繁琐" | 测试比在生产中调试坏技能更不繁琐。 |
| "我确信它很好" | 过度自信保证问题。无论如何都要测试。 |
| "学术审查就够了" | 阅读 ≠ 使用。测试应用场景。 |
| "没时间测试" | 部署未经测试的技能浪费更多时间在后期修复上。 |

**所有这些都意味着：在部署前测试。无例外。**

## 使技能免受合理化理由影响

强制纪律的技能（如 TDD）需要抵抗合理化理由。代理很聪明，在压力下会找到漏洞。

**心理学说明：** 理解为何说服技巧有效，有助于你系统地应用它们。参见 persuasion-principles.md 了解研究基础（Cialdini, 2021; Meincke et al., 2025）关于权威、承诺、稀缺性、社会认同和统一性原则。
...
从基线测试中捕获合理化理由（参见下面的测试部分）。代理提出的每个借口都放入表中：
...
| "后写测试能达到相同目标" | 后写测试 = "这做什么？" 先写测试 = "这应该做什么？" |
```
...
```yaml
description: 在实现任何功能或修复错误前，编写实现代码前使用
```
...
代理找到了新的合理化理由？添加明确的反驳。重新测试直到无懈可击。
...
### ❌ 叙述性示例
"在 2025-10-03 的会话中，我们发现 empty projectDir 导致了..."
**为何差：** 太具体，不可重用
