...
Gemini CLI 没有与 Claude Code 的 `Task` 工具等效的工具。依赖子代理分派的技能（`subagent-driven-development`、`dispatching-parallel-agents`）将通过 `executing-plans` 回退到单会话执行。
...
| `save_memory` | 在会话间将事实持久化到 GEMINI.md |
| `ask_user` | 向用户请求结构化输入 |
